...
function obterPontuacaoMaximaQuiz (idUsuario){
    var instrucaoSql = `SELECT IFNULL(MAX(qtd_pontos), 0) AS 'pontuacaoMaxima' FROM pontuacao_quiz where fk_usuario = ${idUsuario}`
    return database.executar(instrucaoSql)
}
...
function obterTop5LivrosMaisFavoritados (){
    var instrucaoSql = `SELECT livro.titulo, count(idLivro) AS 'quantidade' FROM livro JOIN livro_favoritado ON idLivro = fk_livro GROUP BY livro.titulo ORDER BY count(idLivro) DESC limit 5 `
    return database.executar(instrucaoSql)
}

function obterTop5MaisEmprestados(){
    var instrucaoSql = `SELECT titulo, count(*)  AS 'qtdEmprestimos' From emprestimo INNER JOIN livro ON fk_livro = idLivro GROUP BY titulo  ORDER BY count(*) DESC limit 5`
    return database.executar(instrucaoSql)
}
